...
如果上面 3 步能成功，你再继续看后面的架构、Memory、Reflection、评测和 Docker 部分会更顺手。
...
| `--user-id` | plan/resume | 用户 ID，启用长期记忆与用户画像 |
| `--memory-path` | plan/resume | 自定义 Memory SQLite 路径（默认 data/user_memory.sqlite） |
